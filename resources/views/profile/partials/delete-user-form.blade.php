    ...
    <x-ui.modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
            ...
            <p class="mt-1 text-sm text-gray-600">
                {{ __('Sobald dein Account gelöscht ist, werden alle Ressourcen und Daten dauerhaft gelöscht. Bitte gib dein Passwort ein, um zu bestätigen, dass du deinen Account dauerhaft löschen möchtest.') }}
            </p>

            <div class="mt-6">
                <x-forms.input-label for="password" value="{{ __('Passwort') }}" class="sr-only" />
